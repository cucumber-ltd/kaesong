...
describe('CommandBus', () => {
  ...
  it('lets you inject your own promise to wait until the commands were handled and the events were persisted', async () => {
    await new Promise((resolve, reject) =>
      commandBus.dispatch(new TestSlowCommand(), { resolve, reject })
    )
  ...
  it('rejects the running command when committing fails', async () => {
    ...
    await assertThrows(
      async () =>
        await commandBus.dispatchAndWait(new TestFastCommand(), 'commit fails')
    )
  })

  it('lets you wait for the command handler to reject', async () => {
    await assertThrows(() =>
      commandBus.dispatchAndWait(new FailingCommand(), null, Error)
    )
  })

  it('queues commands when paused and dispatches them when resumed', async () => {
    await commandBus.pause()
    await commandBus.dispatch(new TestFastCommand())
    await commandBus.dispatch(new TestSlowCommand())
    await commandBus.dispatch(new TestFastCommand())
    await sleep(10)
    assert.deepEqual(sideEffects, [])
    await commandBus.resume()
    await eventually(
      () => assert.deepEqual(sideEffects, ['fast', 'slow', 'fast']),
      null,
      10,
      10
    )
