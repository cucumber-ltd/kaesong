...
const asyncPump = promisify(pump)
...
module.exports = class DomainRepository {
  ...
  async replayAllEvents({ end = true, monitor = () => {} } = {}) {
    ...
    return new Promise((resolve, reject) =>
      pump(
        this._eventStore.streamAllEvents(),
        new MarkEventsAsBeingReplayed(),
        ...
        err => {
          ...
          if (!end) {
            log('replayAllEvents end, keeping domain event bus running')
            resolve()
          }
        }
      )
        ...
        .on('end', () => {
          if (end) {
            log('replayAllEvents end, domain event bus closed')
            resolve()
          }
        })
    )
  ...
  async _storeUnitOfWorkEvents(unitOfWork) {
    ...
    await asyncPump(
      unitOfWork.getAllEventsStream(),
      loggingStream,
      this._eventStore.openWriteEventsStream()
    )
  }
