...
const TEST_BAD_UID = 'not-a-real-uid'
...
module.exports = function verifyEventStoreContract(factory) {
  describe('EventStore contract', () => {
    ...
    it('emits errors', callback => {
      const readEvents = []
      const events = eventStore.findEventsByEntityUid(TEST_BAD_UID)
      events.on('error', err => {
        assert(err)
        callback()
      })
    ...
    it('rolls back all events when the first writes ok but second fails', callback => {
      ...
      arrayToStream([
        makeTestEvent(),
        makeTestEvent().with({ entityUid: TEST_BAD_UID }),
      ]).pipe(stream)
    })
