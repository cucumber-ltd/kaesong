...
const PgEventStore = require('.././event_stores/pg_event_store')
const { getPgPool, migrateDb } = require('.././pg')
...
describe('PgEventStore', () => {
  ...
  it('emits an error when a pool exception occurs', callback => {
    const error = new Error('pgPool connect failure')
    const pgPool = {
      connect: fn => process.nextTick(() => fn(error, null, () => {})),
      query: () => {},
      end: () => {},
    }
