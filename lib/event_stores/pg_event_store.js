...
class PgEventStore {
  ...
  findEventsByEntityUid(entityUid) {
    ...
    const pgResultToEventStream = new PgResultToEventStream(this._deserialize)

    queryStream.on('close', () => console.log('close'))

    this._pgPool.connect((err, client, done) => {
      if (err) {
        pgResultToEventStream.emit('error', err)
        return done(err)
      }
      client
        .query(queryStream)
        .pipe(pgResultToEventStream)
        .on('end', () => {
          console.log('end 1')
          queryStream.close()
          client.release()
