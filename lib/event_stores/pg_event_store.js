...
class PgResultToEventStream extends Stream.Transform {
  ...
  _transform(row, _, callback) {
    try {
      ...
      err.message += ` - The ${row.type} event with sequence number ${
        row.sequence_number
      } for entityUid ${row.entity_uid} payload is ${JSON.stringify(row.data)}.`
      callback(err)
    }
  }
}
...
class PgWriteEventsStream extends Stream.Writable {
  ...
  async _writeEvent(event) {
    ...
    await this._query({
      text: `INSERT INTO ${
        this._tableName
      } (entity_uid, entity_version, timestamp, type, data) VALUES ($1, $2, $3, $4, $5)`,
      values: [
        event.entityUid,
        event.entityVersion,
...
class PgEventStore {
  ...
  openWriteEventsStream() {
    ...
    return new PgWriteEventsStream(
      this._pgPool,
      this._tableName,
      this._logError
    )
  ...
  findEventsByEntityUid(entityUid) {
    ...
    const queryStream = new QueryStream(
      `SELECT * FROM ${
        this._tableName
      } WHERE entity_uid=$1 ORDER BY sequence_number`,
      [entityUid]
    )
    const pgQueryStream = new PgQueryStream(this._pgPool, queryStream)
    const resultStream = new PgResultToEventStream(this._deserialize)
    pgQueryStream.on('error', err => resultStream.destroy(err))
    return pgQueryStream.pipe(resultStream)
  }
  ...
  streamAllEvents() {
    ...
    const resultStream = new PgResultToEventStream(this._deserialize)
    pgQueryStream.on('error', err => resultStream.destroy(err))
    resultStream.on('error', () => pgQueryStream.destroy())
    return pgQueryStream.pipe(resultStream)
  }
  ...
  async renameTable(tableName) {
    ...
    await this.query(
      `ALTER SEQUENCE ${
        this._tableName
      }_sequence_number_seq RENAME TO ${tableName}_sequence_number_seq`
    )
    await this.query(
    ...
    await this.query(
      `ALTER INDEX ${
        this._tableName
      }_entity_uid_entity_version_unique RENAME TO ${tableName}_entity_uid_entity_version_unique`
    )
